  ...
  return (
    <div className='flex flex-col items-center justify-center mt-4 w-full h-full' >
      ...
      {a2?.map((item, index) => 
                ...
                className='dragStart text-2xl transform hover:bg-purple-900 transition duration-500 hover:scale-105 font-bold h-24 w-24 bg-purple-600 border-2 border-purple-900 my-1  rounded-lg text-white'>{item}</button>
            )}
      </div>
    </div>
  )
